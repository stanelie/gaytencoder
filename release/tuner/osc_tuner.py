...
def announce_listener():
    ...
    while True:
        ...
        if address == "/encoder/announce" and len(args) >= 7:
            with _devices_lock:
                _devices[sender[0]] = {
                    "prefix": args[0],
                    "dest": args[1],
                    "destPort": int(args[2]),
                    "encoderOk": bool(args[3]),
                    "position": args[4],
                    "lead": args[5],
                    "window": args[6],
                    "at": time.monotonic(),
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .bad { color:#ff8080 !important; }
  .fault { background:#3a1c1c; border:1px solid #6b2b2b; color:#ffb3b3;
...
<body><div class="wrap">
  ...
  <div class="card">
    ...
    <div class="fault" id="faultBanner" style="display:none">
      <b>Encoder not responding.</b> This board is on the network and will accept
      settings, but it is reading nothing from the sensor &mdash; check the RS485
      wiring and that the encoder has power. Its LED is blinking red. It will
      pick up on its own once the encoder answers.
    </div>
  ...
  <div class="card">
    <div class="route">
      ...
      <button id="applyBtn" onclick="applyRouting()">Apply</button>
    </div>
...
// Nothing is shown until a board has actually told us its settings. Starting
// from hardcoded defaults would display values no board necessarily holds,
// and the first drag would then jump from a number that was never real.
const CONTROLS = ['lead','leadN','window','windowN','prefix','dest','destPort','applyBtn'];
function setControlsEnabled(on) { CONTROLS.forEach(id => $(id).disabled = !on); }
function clearControls() {
  $('leadN').value = ''; $('windowN').value = '';
  $('prefix').value = ''; $('dest').value = ''; $('destPort').value = '';
}
...
// Never overwrite a control the operator is currently touching - a slider
// being dragged or a field being typed into holds focus.
function busy(id) { return document.activeElement === $(id); }

// Adopt the values the board reports as the truth. Called on selection and
// again from each beacon, so a board that reboots, or is changed from another
// copy of this panel, is reflected here rather than silently disagreeing.
function adoptBoardValues(dev, force) {
  if (force || (!busy('lead') && !busy('leadN'))) {
    $('lead').value = dev.lead;
    $('leadN').value = dev.lead.toFixed(3);
  }
  if (force || (!busy('window') && !busy('windowN'))) {
    $('window').value = dev.window;
    $('windowN').value = dev.window.toFixed(3);
  }
  if (force || !busy('prefix'))   $('prefix').value = dev.prefix;
  if (force || !busy('dest'))     $('dest').value = dev.dest;
  if (force || !busy('destPort')) $('destPort').value = dev.destPort;
}

function selectDevice(dev) {
  selected = dev.ip;
  adoptBoardValues(dev, true);
  ...
  list.innerHTML = devices.map((d, i) =>
    `<div class="dev ${d.ip === selected ? 'sel' : ''}" onclick="selectDevice(lastDevices[${i}])">
       ...
       ${d.encoderOk
          ...
          : `<span class="pos bad">encoder not responding</span>`}
     </div>`).join('');
}
...
async function refreshDevices() {
  try {
    ...
    const current = j.devices.find(d => d.ip === selected);
    if (current) {
      adoptBoardValues(current, false);
      $('faultBanner').style.display = current.encoderOk ? 'none' : 'block';
    } else if (selected) {
      // The selected board stopped announcing - do not keep offering
      // controls that have nothing to act on.
      selected = null;
      setControlsEnabled(false);
      clearControls();
      $('faultBanner').style.display = 'none';
      log('selected board stopped announcing', 'err');
    }
