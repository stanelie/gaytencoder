...
#
#   /encoder/announce  ,ssiifff
#       <prefix> <dest> <dest port> <encoder ok> <position> <lead> <window>
#
# "encoder ok" is 1 when the encoder is answering and 0 when it is not, so the
# tuner can explain a board that is present but producing nothing. A board with
# a dead or unplugged encoder still comes up, still appears here, and still
# accepts configuration - it just cannot stream.
#
